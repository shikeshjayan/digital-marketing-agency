...
export default function NotificationDropdown() {
  ...
  const unreadCount = lastViewed
    ? notifications.filter(
        (n) => new Date(n.time).getTime() > new Date(lastViewed).getTime(),
      ).length
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        type="button"
        className="relative p-2.5 min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg hover:bg-surface transition-colors duration-150 cursor-pointer"
      ...
      {isOpen && (
        <div className="fixed sm:absolute left-4 right-4 sm:left-auto sm:right-0 sm:translate-x-0 mt-2 sm:w-80 bg-background border border-border rounded-lg shadow-lg z-50 overflow-hidden top-16 sm:top-auto sm:mt-2">
              ...
              notifications.map((n, i) => {
                ...
                return (
                  <button
                    key={`${n.type}-${n.id}-${i}`}
                    type="button"
                    className={`w-full flex items-start gap-2.5 sm:gap-3 px-3 sm:px-4 py-3 hover:bg-surface transition-colors duration-150 text-left border-b border-border last:border-0 cursor-pointer ${isUnread ? "bg-primary-light/40" : ""}`}
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 sm:gap-2">
                        ...
                        <span className="text-[11px] sm:text-xs text-muted flex-shrink-0">
                          {timeAgo(n.time)}
                        </span>
                      </div>
                      ...
                      {n.preview && (
                        <div className="text-[11px] sm:text-xs text-muted mt-0.5 truncate">
                          "{n.preview}..."
                        </div>
                      )}
                    </div>
          ...
          {notifications.length > 0 && (
            <div className="border-t border-border px-3 sm:px-4 py-2">
              <button
                type="button"
                className="w-full text-center text-sm text-primary hover:text-primary-hover font-medium py-2.5 min-h-[44px] cursor-pointer"
                onClick={() => {
                  setIsOpen(false);
                  setLastViewed();
                  setLastViewedState(new Date().toISOString());
