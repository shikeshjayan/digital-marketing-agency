          ...
          <div className="flex items-center md:w-64 shrink-0">
            ...
            >
              <span className="flex flex-col items-center justify-center w-5 h-5 gap-1">
                <span className={`block h-0.5 bg-heading rounded-full transition-all duration-200 ${sidebarOpen ? 'w-5 rotate-45 translate-y-[3px]' : 'w-5 -translate-y-[3px]'}`} />
                <span className={`block h-0.5 bg-heading rounded-full transition-all duration-200 ${sidebarOpen ? 'w-5 opacity-0' : 'w-5 opacity-100'}`} />
                <span className={`block h-0.5 bg-heading rounded-full transition-all duration-200 ${sidebarOpen ? 'w-5 -rotate-45 -translate-y-[3px]' : 'w-3.5 translate-y-[3px]'}`} />
              </span>
            </button>
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-4 flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:block">
                <div className="small-text font-semibold text-heading leading-tight">{profile?.name || 'Admin'}</div>
                <div className="small-text text-muted">{profile?.role || 'Administrator'}</div>
              </div>
            </div>
          </div>
        </div>
